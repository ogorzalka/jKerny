...
(function ($) {
    ...
    $.jkerny = {
        ...
        applyProperties: function(selectors, selector, type) {
            ...
            for (var item in split_selector) {
                var elements = split_selector[item].replace(new RegExp(jkerny.pseudo_selectors), '').split(jkerny.split_expr)[0];
                if (type == 'pseudo-selector') {
                    mainSelector = $(split_selector[item].replace(new RegExp(jkerny.pseudo_selectors), ''));
                    for (var i = 0; i < mainSelector.length; i++) {
                        convertedSelector = split_selector[item].replace(new RegExp('span.jkerny-(letter|word)('+jkerny.pseudo_structural+')?', 'gi'), '.jkerny-$1-'+jkerny.kerning_counter);
                        $this = $(mainSelector[i]);
                        $this.addClass('jkerny-'+($this.is('.jkerny-letter') ? 'letter' : 'word')+'-'+jkerny.kerning_counter).parent('.jkerny-word').addClass('jkerny-word-'+jkerny.kerning_counter);
                    ...
                    var kerningValues = selectors[selector]['-jkerny-'+type+'-spacing'].replace(/[\n\s]+/g, ' ').split(' '); 
                    for (var j = 0; j < kerningValues.length; j++) {
                        mainSelector = $(elements).find('span.jkerny-'+type+':eq('+j+')');
                        for (var k = 0; k < mainSelector.length; k++) {
                            if (typeof kerningValues[j] != 'undefined') {
                                $(mainSelector[k]).addClass('kerning-'+jkerny.kerning_counter);
