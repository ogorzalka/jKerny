...
(function ($) {
    ...
    $.jkerny = {
        lettering_replace: {
            ...
            '(span\.jkerny-word([^ ]*)?) *span\.jkerny-word:(first|last)-child'  : '$1$2' // prevent double span.jkerny-word
        },
        ...
        adjustKerning: function(selectors, selector, type) {
            if (typeof selectors[selector]['-jkerny-'+type+'-spacing'] != 'undefined') {
                var jkerny = this,
                    ...
                    elements = selector.replace(this.pseudo_selectors, '').split(jkerny.split_expr)[0]; 
                ...
                for (var i = 0; i <= kerningValues.length; i++) {
                    ...
                    $letterSelector.each(function(j, item) {
                        if (typeof kerningValues[i] != 'undefined') {
                            ...
                            selectors[elements+' .jkerny-'+type+'.kerning-'+jkerny.kerning_counter] = { 'margin-right' : kerningValues[i] };
        ...
        proceedCss: function (selectors) {
			...
			if (jkerny.getLength(selectors) > 0) {
	            for (var selector in selectors) {
	                var elements = selector.replace(this.pseudo_selectors, '').split(jkerny.split_expr)[0];
	                $(elements)
	                	.not('.jkerny')
	                		.addClass('jkerny')
	                		.lettering('words')
