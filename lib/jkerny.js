...
(function ($) {
    ...
    $.jkerny = {
        ...
        pseudo_structural:':(eq|empty|(first|last|only|nth(-last)?)-(child|of-type))([^\\)]*\\))?', // courtesy of selectivizr by keithclark http://selectivizr.com
        ...
        parseMediaQueries: function(content) {
            ...
            if (media_datas) {
	            for (var i = 0; i < media_datas.length; i++) {
	                jkerny.medias_screen = $.trim(media_datas[1]);
        ...
        applyProperties: function(selectors, selector) {
            var jkerny, $mainSelector, convertedSelector, elements, kerningValues, selectorType, split_selector, i, j;
            jkerny = this;
        ...
        filterSelectors: function (selectors) {
            ...
            for (var selector in s) {
                ...
                for (var attribute in s[selector]) {
                    if (attrMatch = attribute.match(new RegExp(/-jkerny-(letter|word)-spacing/), 'gi')) {
